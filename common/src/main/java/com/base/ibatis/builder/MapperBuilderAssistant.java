...
//        boolean isSelect = sqlCommandType == SqlCommandType.SELECT;
        String lineStr = String.valueOf(context.getNode().getUserData(XmlLocateHandler.LINE_NUMBER_KEY_NAME));
        Integer line = StringUtils.isBlank(lineStr) ? null : Integer.parseInt(lineStr);
        final Object userData = context.getNode().getUserData(XmlLocateHandler.LINE_NUMBER_KEY_NAME);
        MappedStatement stmt = new MappedStatement.Builder(configuration, id, sqlSource, sqlCommandType)
                .resource(resource)
                .line(line)
                .build();

        configuration.addMappedStatment(stmt);
        return stmt;
    }
